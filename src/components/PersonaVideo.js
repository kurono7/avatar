...
import { headerHeight, transparentHeader,landingBackgroundImage, landingBackgroundColor  } from '../config';
...
const PersonaVideo = ({
  ...
  const handleResize = () => {
    if (containerRef.current) {
      ...
      setVideoDimensions('50%', videoHeight);
    ...
    <div ref={containerRef} className={className} style={{ height}}>
      {
        connected
          ? (
            <video ref={videoRef} autoPlay  className="persona-video" id="personavideo" data-sm-video/>
          )
          : null
      }
      {
